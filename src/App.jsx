...
export default function App() {
  ...
  function handleDeleteChat() {
    axios
      .delete(`http://localhost:3001/chats/${chatsState.selectedChatId}`)
      .then((response) => {
        setChatsState((prevState) => {
  ...
  function handleSendMessage(chat, message, sender) {
    ...
    axios
      .post(`http://localhost:3001/chats/${chatsState.selectedChatId}`, {
  ...
  function handleEnterUsername() {
    if (usernameInput.current && usernameInput.current.value.trim() !== "") {
      ...
      axios
        .get(`http://localhost:3001/chats/${enteredUsername}`)
        .then((response) => {
